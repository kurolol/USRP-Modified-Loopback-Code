	...
	uhd::tx_streamer::sptr tx_stream, const std::string& file, size_t samps_per_buff)
{
	...
	while (not stop_signal_called) {
		...
		//if end of file
		if (infile.eof()) {
			//close and reopen the file to repeat the signal
			infile.close();
			std::ifstream infile(file.c_str(), std::ifstream::binary);
		}
	}
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
	...
	std::string tx_args, tx_file, wave_type, tx_ant, tx_subdev, ref, otw, tx_channels;
	...
	desc.add_options()
		...
		("tx-file", po::value<std::string>(&tx_file)->default_value("test_FMCW.dat"), "name of the file to read binary samples from")
		("rx-file", po::value<std::string>(&rx_file)->default_value("usrp_samples.dat"), "name of the file to write binary samples to")
	...
	transmit_thread.create_thread(boost::bind(
		&send_from_file, tx_stream, tx_file, spb));
